...
// PATCH /api/businesses/[businessId]/locations/[locationId]/items/[itemId]/options/[optionId]/move-down
export async function PATCH(
  ...
  try {
    ...
    /*
     * Find the closest Option below this one.
     */
    const belowOption = await prisma.itemOption.findFirst({
      where: {
        itemId,
        order: {
          gt: currentOption.order,
        },
      },
      orderBy: {
        order: "asc",
    ...
    const belowOrder = belowOption.order;
    ...
    console.error("Failed to move item option down:", error);

    return NextResponse.json(
      { error: "Failed to move item option down" },
      { status: 500 },
    );
  }
}
