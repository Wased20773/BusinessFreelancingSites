...
describe("getSlug", () => {
  ...
  it("throws when the business slug is missing", () => {
    ...
    expect(() => getSlug(request)).toThrow("Missing business slug");
  });
});
...
describe("getNextOrder", () => {
  it("returns 1 when no existing records are found", async () => {
    ...
    const result = await getNextOrder(model, where);
  ...
  it("returns 500 when determining the next order fails", async () => {
    ...
    const result = await getNextOrder(model, {
      businessId: "business-123",
    });
    ...
    expect(responseBody).toEqual({
      error: expect.stringContaining("Failed to determine the next order"),
    });
...
describe("getBusinessResponse", () => {
  ...
  it("successfully returns the selected business data", async () => {
    ...
    mockPrisma.business.findUnique.mockResolvedValue(business as never);
...
describe("imageRequestValidation", () => {
  it("returns 500 when the request body cannot be parsed as form data", async () => {
    ...
    expect(responseBody).toEqual({
      error: expect.stringContaining("Failed to get form data from request"),
    });
  });
  ...
  it("returns 400 when the uploaded image is empty", async () => {
    ...
    const emptyImage = new File([], "empty.png", {
      type: "image/png",
    });
...
describe("normalizeDayOfWeek", () => {
  ...
  it.each(["", "mon", "weekday", "holiday", "123", "monday ", " monday"])(
    'returns null for "%s"',
    (input) => {
...
describe("checkTimeOverlap", () => {
  it.each([
    ...
    (newOpenTime, newCloseTime, existingOpenTime, existingCloseTime) => {
      expect(
        checkTimeOverlap(
          newOpenTime,
          newCloseTime,
